...
import shutil
import random
from Trainer.PO_models import DCOL
import shutil
...
parser.add_argument("--mu", type=float, help="regularization paraemter", default= 20., required=False)
...
outputfile = "Rslt/DCOL_matching{}_index{}.csv".format(args.instance, args.index)
regretfile = "Rslt/DCOL_matchingRegret{}_index{}.csv".format(args.instance, args.index)
ckpt_dir =  "ckpt_dir/DCOL{}_index{}/".format(args.instance, args.index)
log_dir = "lightning_logs/DCOL{}_index{}/".format(args.instance, args.index)
learning_curve_datafile = "LearningCurve/DCOL{}_mu{}_lr{}_batchsize{}_seed{}_index{}.csv".format(args.instance,mu,lr,batch_size,seed, args.index)
...
for seed in range(10):
    ...
    model = DCOL(solver,lr=lr, mu=mu,seed=seed)
    ...
    model = DCOL.load_from_checkpoint(best_model_path ,solver=solver,lr=lr, mu=mu,seed=seed)    
