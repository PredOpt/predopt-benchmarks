...
######################################  Data Reading #########################################
df = pd.read_csv("synthetic_path/data_N_1000_noise_0_deg_2.csv")
...
def model_tune(config,train_dl, valid_dl, solpool=None,num_epochs=30, num_gpus=0):
    ...
    model = FenchelYoung(net=nn.Linear(5,1),l1_weight = config['l1_weight'],sigma=100,   seed=random.randint(0,10)) 
