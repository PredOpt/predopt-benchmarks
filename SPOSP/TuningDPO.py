...
def model_tune(config,train_dl, valid_dl, solpool=None,num_epochs=30, num_gpus=0):
    ...
    model = FenchelYoung(net=nn.Linear(5,1),sigma=config['sigma'],  l1_weight=config['l1_weight'], seed=random.randint(0,10)) 
...
def tune_model_asha(train_dl, valid_dl,solpool=None,num_samples=3, num_epochs=30, gpus_per_trial=0):
    ### ***** Model Specific config *****
    config = {
            "l1_weight":tune.grid_search([10**(k) for k in range(-5,3)]),
            "sigma":tune.grid_search([0.1,0.5,1,10,1e-3]),
        }
    ...
    reporter = CLIReporter(
        ### ***** Model Specific parameter *****
            parameter_columns=[ "l1_weight","sigma" ],
    ...
    print( result_df.groupby(['config.l1_weight','config.sigma']).agg({"regret":['mean','std'],
