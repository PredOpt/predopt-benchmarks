...
def seed_all(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
outputfile = "Rslt/UnitNormed__Twostage_rslt.csv"
regretfile= "Rslt/UnitNormed__Twostage_Regret.csv"
ckpt_dir =  "ckpt_dir/UnitNormed__Twostage/"
log_dir = "lightning_logs/UnitNormed__Twostage/"
    ...
    for deg in [1,2,4, 6,8 ]:
        for noise in [0.5,0.0]:
            ...
            for seed in range(10):
                ...
                model = twostage_regression(net= net, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs)
                trainer.fit(model, datamodule=data)
                best_model_path = checkpoint_callback.best_model_path
                model = twostage_regression.load_from_checkpoint(best_model_path,
                net= net, lr= lr, seed=seed)
            ...
            "val_mse": mses })
            df['model'] ='Twostage'
            df.to_csv("LearningCurve/UnitNormed__Twostage_data_N_{}_noise_{}_deg_{}_lr{}.csv".format(N,noise,deg,lr))