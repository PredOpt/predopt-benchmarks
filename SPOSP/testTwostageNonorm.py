...
def seed_all(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
outputfile = "Rslt/NoNormed_Twostage_rslt.csv"
regretfile= "Rslt/NoNormed_Twostage_Regret.csv"
ckpt_dir =  "ckpt_dir/NoNormed_Twostage/"
log_dir = "lightning_logs/NoNormed_Twostage/"
...
for N in [1000,100]:
    for deg in [1,2,4, 6,8 ]:
        for noise in [0.5,0.0]:
            ...
            "val_mse": mses })
            df['model'] ='Twostage'
            df.to_csv("LearningCurve/NoNormed_Twostage_data_N_{}_noise_{}_deg_{}_lr{}.csv".format(N,noise,deg,lr))