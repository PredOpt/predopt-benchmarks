...
class GurobiShortestPath:
    ...
    def multi_shortest_path(self, source, target):
...
def generate_instance(datapoints, p=5, deg=2, noise=0.5):
    file_output = 'synthetic_path/data_N_{0}_noise_{1}_deg_{2}.csv'.format(
        datapoints, noise, deg)
    ...
    for i in range(datapoints):
        ...
        for (u, v) in E:
            ...
            pred = beta[(u, v)][0] + sum(x[u, v][k] * beta[(u, v)][k + 1]
                                         for k in range(p))
            c[u, v] = round((1 + 1 / math.sqrt(p) * (pred + 3)**deg)
                            * random.uniform(1 - noise, 1 + noise), 5)

            attributes_string = ','.join(str(x[u, v][k]) for k in range(p))

            ff.write('{0},{1},{2},{3},{4}\n'.format(
                i, u, v, c[u, v], attributes_string))
...
def compute_shortest_path(data_file):
    ...
    with open(data_file, newline='') as csvfile:
        ...
        for row in reader:
            ...
            c[int(row['data']), (int(row['node_init']),
                                 int(row['node_term']))] = float(row['c'])
    ...
    ff = open(data_sol, 'w')
    ff.write('i,z,n_optimal,' +
             ','.join(['w_{0}'.format(e) for e in E]) + '\n')
    ...
    for i in N:
        for e in E:
            ...
            if i == 0:
                print(e, c[i, e])
        graph = GurobiShortestPath(G)
        n_optimal = graph.multi_shortest_path(source=s, target=t)
        ...
        ff.write(','.join([str(i), str(z), str(n_optimal)] +
                 [str(w[e]) for e in E]) + '\n')
        if n_optimal > 1:
            print(
                'Non unique solutuins for datafile {} instance {} edge {}'.format(
                    data_file, i, e))
        # if i==0:
        #     print('PAth 0', z,path)
...
if __name__ == '__main__':
    ...
    SETTINGS = [(n, noise, deg)
                for n in ns for noise in noises for deg in degs]
