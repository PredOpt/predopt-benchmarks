...
outputfile = "Rslt/UnitNormed_Listwise_rslt.csv"
regretfile= "Rslt/UnitNormed_Listwise_Regret.csv"
ckpt_dir =  "ckpt_dir/UnitNormed_Listwise/"
log_dir = "lightning_logs/UnitNormed_Listwise/"
    ...
    for noise in [0.0,0.5]:
        for deg in [2,4,6,8,1]:
            ...
            for seed in range(10):
                ...
                model = CachingPO(init_cache=init_cache,loss_fn=Listnet_loss, tau=temperature, net= net, 
                growth= growth, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs)
                trainer.fit(model, datamodule=data)
                best_model_path = checkpoint_callback.best_model_path
                model = CachingPO.load_from_checkpoint(best_model_path,
                init_cache=init_cache,loss_fn=Listnet_loss, tau=temperature,net= net, 
                growth= growth, lr= lr,l1_weight=l1_weight, seed=seed, )
            ...
            "val_mse": mses })
            df['model'] ='Listwise'
            df.to_csv("LearningCurve/UnitNormed_Listwise_data_N_{}_noise_{}_deg_{}_growth{}_lr{}_temp{}.csv".format(N,noise,deg,growth,lr, temperature))