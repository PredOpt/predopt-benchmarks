...
outputfile = "Rslt/UnitNormed_DPO_rslt.csv"
regretfile= "Rslt/UnitNormed_DPO_Regret.csv"
ckpt_dir =  "ckpt_dir/UnitNormedDPO/"
log_dir = "lightning_logs/UnitNormedDPO/"
...
for noise in [0.0,0.5]:
    for deg in [4,8,6,2,1]:
        for sigma in [0.5,1,0.1,2,5]:
            ###################################### Hyperparams #########################################
            lr = 0.1
            l1_weight = 1e-5
            batchsize  = 128
            ...
            for seed in range(10):
                ...
                model = FenchelYoung(net= net, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs,sigma=sigma, num_samples= num_samples)
                trainer.fit(model, datamodule=data)
                best_model_path = checkpoint_callback.best_model_path
                model = FenchelYoung.load_from_checkpoint(best_model_path,
                net= net, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs,sigma=sigma, num_samples= num_samples)
            ...
            "val_mse": mses })
            df['model'] ='DPO'
            df.to_csv("LearningCurve/UnitNormed_DPO_data_N_{}_noise_{}_deg_{}_lr{}_sigma{}_numsamples{}.csv".format( N,noise,deg,lr, sigma, num_samples))