...
outputfile = "Rslt/Normed_Intopt_rslt.csv"
regretfile= "Rslt/Normed_Intopt_Regret.csv"
ckpt_dir =  "ckpt_dir/Intopt/"
log_dir = "lightning_logs/Intopt/"
...
for seed in range(10):
    ...
    model = IntOpt(net= net,thr=thr, damping=damping, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs)
    trainer.fit(model, datamodule=data)
    best_model_path = checkpoint_callback.best_model_path
    model = IntOpt.load_from_checkpoint(best_model_path,
    net= net,thr=thr, damping=damping, lr= lr, l1_weight=l1_weight, seed=seed)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Intopt'
df.to_csv("LearningCurve/Intopt_data_N_{}_noise_{}_deg_{}_lr{}_thr{}_damping{}.csv".format(N,noise,deg,lr,thr, damping))