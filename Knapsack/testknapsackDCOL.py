...
parser.add_argument("--mu", type=float, help="regularization parameter", default= 1.)
...
outputfile = "Rslt/DCOL_index{}.csv".format( args.index)
regretfile = "Rslt/DCOL_Regretindex{}.csv".format( args.index)
ckpt_dir =  "ckpt_dir/DCOL_index{}/".format( args.index)
log_dir = "lightning_logs/DCOL_index{}/".format( args.index)
learning_curve_datafile = "LearningCurve/DCOLcapa{}_mu{}_lr{}_batchsize{}_index{}.csv".format(capacity,mu,lr,batch_size, args.index)
...
for seed in range(10):
    ...
    model =  DCOL(weights,capacity,n_items,mu=mu, lr=lr, seed=seed)
    trainer.fit(model, datamodule=data)
    best_model_path = checkpoint_callback.best_model_path
    model = DCOL.load_from_checkpoint(best_model_path,
        weights = weights,capacity= capacity,n_items = n_items,mu=mu,lr=lr, seed=seed)
    ##### SummaryWrite ######################
