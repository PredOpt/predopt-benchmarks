...
from Trainer.PO_models import multilayer_mse
...
parser.add_argument("--n_layers", type=int, help="Number of layers", default= 3, required=False)
parser.add_argument("--n_hidden", type=int, help="Number of neurons in hidden layers", default= 4, required=False)
...
############### Configuration
###################################### Hyperparams #########################################
n_layers, n_hidden = args.n_layers,  args.n_hidden
lr = args.lr
...
outputfile = "Rslt/TwostageMSEmultilayer_index{}.csv".format( args.index)
regretfile = "Rslt/TwostageMSEmultilayer_Regretindex{}.csv".format( args.index)
ckpt_dir =  "ckpt_dir/TwostageMSEmultilayer_index{}/".format( args.index)
log_dir = "lightning_logs/TwostageMSEmultilayer_index{}/".format( args.index)
learning_curve_datafile = "LearningCurve/TwostageMSEmultilayercapa{}_layer{}_hidden{}_lr{}_batchsize{}_index{}.csv".format(capacity ,n_layers, n_hidden, lr,batch_size, args.index)
...
for seed in range(10):
    ...
    model =  multilayer_mse(weights,capacity,n_items,lr=lr, seed=seed,  n_hidden=n_hidden, n_layers=n_layers)
    trainer.fit(model, datamodule=data)
    best_model_path = checkpoint_callback.best_model_path
    model = multilayer_mse.load_from_checkpoint(best_model_path,n_hidden=n_hidden, n_layers=n_layers,
